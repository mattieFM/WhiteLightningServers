...
var files = fs.readdirSync(Config.path + "/sadpup")
let chosenfile = Config.path +  "/sadpup/" +files[Math.floor(Math.random() * files.length)];
...
message.channel.send(`You are the chosen one of the \"Sad Pup\" emotes`, {
    ...
    file: Config.path +  + "/sadpup/" +files[Math.floor(Math.random() * files.length)]
});
}
