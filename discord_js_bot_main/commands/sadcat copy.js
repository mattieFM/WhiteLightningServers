...
var files = fs.readdirSync(Config.path + "/happup")
let chosenfile = Config.path + "/happup/" +files[Math.floor(Math.random() * files.length)];
...
message.channel.send(`Not WoofPup`, {
    ...
    file: Config.path  + "/happup/" +files[Math.floor(Math.random() * files.length)]
});
}

if(message.author.id == wolfpup){
    message.channel.send(`WoofPup`, {
