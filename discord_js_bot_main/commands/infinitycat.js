...
var fs = require('fs');
var files2 = fs.readdirSync(Config.path + "/infdog")
var files = fs.readdirSync(Config.path + "/infinity")
...
message.channel.send(`\"cat\"`, {
    ...
    file: Config.path + "/infinity/" +files[Math.floor(Math.random() * files.length)]
});
}
...
if(message.author.id == wolfpup){
    message.channel.send("you are woofpup? \"dog\"",{
        ...
        file: Config.path +  "/infdog/" +files2[Math.floor(Math.random() * files.length)]
    });
