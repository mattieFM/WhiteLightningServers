...
var files = fs.readdirSync(Config.path + "/image_commands/sadcat")
let chosenfile = Config.path +  "/image_commands/sadcat/" +files[Math.floor(Math.random() * files.length)];
...
message.channel.send(`You are the chosen ones of the \"Sad Cat\" emotes`, {
    ...
    file: Config.path  + "/image_commands/sadcat/" +files[Math.floor(Math.random() * files.length)]
});
}
